...
class MainActivity : AppCompatActivity() {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        if (item.itemId == R.id.add_post_menu){
            val action = ArtListFragmentDirections.actionArtListToDetailFragment("new", 0)
            Navigation.findNavController(this@MainActivity, R.id.fragment).navigate(action)
        }
